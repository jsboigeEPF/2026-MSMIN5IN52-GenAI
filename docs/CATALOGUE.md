...
Ce premier lot propose **25 sujets** répartis en neuf catégories. Chaque sujet définit un MVP réalisable en six semaines, un protocole d'évaluation et un plan de repli. Les notebooks liés sont des ressources de démarrage : le livrable doit aller au-delà de leur reproduction.
...
| Catégorie | Sujets |
|---|---|
| A — Texte structuré, RAG et mémoire | [A1](#a1--rag-hybride-avec-reranking-et-citations) · [A2](#a2--mémoire-persistante-multi-sessions) · [A3](#a3--contrats-json-sous-stress) · [A4](#a4--mémoire-multimodale-évaluable) |
| B — Agents, orchestration et MCP | [B1](#b1--processus-agentique-avec-validation-humaine) · [B2](#b2--serveur-mcp-métier-audité) · [B3](#b3--orchestrateur-agentique-sous-budget) · [B4](#b4--générateur-auto-validant-de-notebooks) |
| C — Images | [C1](#c1--benchmark-dévaluation-dédition-dimages) · [C2](#c2--fabrique-de-schémas-pédagogiques-cohérents) |
| D — Audio et voix | [D1](#d1--agent-vocal-temps-réel-sous-contrainte-de-latence) · [D2](#d2--doublage-multi-locuteurs-mesuré) |
| E — Vidéo | [E1](#e1--micro-capsules-vidéo-pédagogiques) · [E2](#e2--benchmark-de-compréhension-temporelle-vidéo) |
| F — Plateformes et production | [F1](#f1--qa-e2e-dopen-webui) · [F2](#f2--rag-wordpress-isolé-et-mesuré) · [F3](#f3--application-genai-observable-avec-net-aspire) |
| G — Ingénierie assistée | [G1](#g1--banc-dessai-dagents-de-codage) · [G2](#g2--garde-fous-roslyn-pour-code-généré) |
| H — Évaluation, sécurité et fiabilité | [H1](#h1--laboratoire-red-team-dinjection-de-prompt) · [H2](#h2--harness-dévaluation-dagents) · [H3](#h3--observatoire-du-reward-hacking) |
| I — Adaptation et inférence | [I1](#i1--qlora-sans-contamination-de-lévaluation) · [I2](#i2--inférence-locale-net-face-aux-api-cloud) · [I3](#i3--frontière-coût-précision-du-test-time-compute) |
...
- **Budget** : ordre de grandeur à confirmer par le groupe avant les campagnes d'essais.
...
- **Faisabilité** : base locale ou Qdrant ; GPU non requis ; données synthétiques obligatoires. Plan B : embeddings locaux.
...
**Références** : [MemGPT](https://arxiv.org/abs/2310.08560) · [Qdrant — Payload](https://qdrant.tech/documentation/concepts/payload/) · [OWASP — LLM02 Sensitive Information Disclosure](https://genai.owasp.org/llmrisk/llm02-insecure-output-handling/)
...
- **Faisabilité** : Semantic Kernel Python ou .NET ; GPU non requis. Plan B : outils entièrement simulés.
...
- **Faisabilité** : SDK MCP Python/TypeScript/C# ; aucun GPU ; API facultative. Plan B : client de test déterministe.
...
- **Faisabilité** : Jupyter + Semantic Kernel ; exécution limitée et sans accès aux secrets. Plan B : exercices Python sans réseau.
...
- **Faisabilité** : API ou ComfyUI ; GPU local non requis. Plan B : petit corpus pré-généré à corriger.
...
- **Faisabilité** : Whisper + TTS ; utiliser des voix consenties ou synthétiques. Plan B : TTS local Kokoro et extrait libre.
...
- **Faisabilité** : génération coûteuse et lente ; plafonner les scènes. Plan B : image-to-video ou slideshow animé.
...
**Références** : [Qwen2.5-VL](https://github.com/QwenLM/Qwen2.5-VL) · [Video-MME](https://arxiv.org/abs/2405.21075) · [MVBench](https://arxiv.org/abs/2311.17005)
...
- **Faisabilité** : Docker + navigateur ; GPU inutile. Plan B : réponses de modèle mockées.
...
- **Faisabilité** : Docker WordPress ; GPU inutile ; petit budget embeddings. Plan B : corpus et embeddings locaux.
...
- **Faisabilité** : .NET Aspire + conteneurs ; GPU inutile. Plan B : connecteur modèle simulé.
...
- **Faisabilité** : .NET uniquement, API facultative, aucun GPU. Plan B : corpus synthétique versionné.
...
- **Faisabilité** : commencer avec l'environnement jouet ; GPU facultatif pour un petit modèle. Plan B : politique tabulaire ou modèle minuscule.
...
- **Faisabilité** : Colab ou GPU partagé, petit modèle uniquement ; aucun GPU requis pour la démo si l'adapter est sauvegardé. Plan B : entraînement très réduit.
...
- **Faisabilité** : modèles compacts ; CPU accepté. Plan B : GGUF très quantifié et sous-échantillon.

**CoursIA** : [LLamaSharp Bake-Off](https://github.com/jsboige/CoursIA/blob/main/MyIA.AI.Notebooks/GenAI/Texte/10e_LLamaSharp_DotNet_BakeOff.ipynb) · [ORT GenAI Bake-Off](https://github.com/jsboige/CoursIA/blob/main/MyIA.AI.Notebooks/GenAI/Texte/10f_ORTGenAI_DotNet_BakeOff.ipynb) · [Quantization](https://github.com/jsboige/CoursIA/blob/main/MyIA.AI.Notebooks/GenAI/Texte/11_Quantization.ipynb)
